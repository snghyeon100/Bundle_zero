...
def main():
    ...
    parser.add_argument("--fileA", required=True, help="방법론 A의 결과 CSV 파일 경로 (기준점)")
    parser.add_argument("--fileB", required=True, help="방법론 B의 결과 CSV 파일 경로 (새로운 프롬프트 등)")
    parser.add_argument("--name", required=True, help="출력 파일명 접두사 (예: pog_base_vs_hn)")
    args = parser.parse_args()

    # Load dataframes
    print(f">>> Loading File A: {args.fileA}")
    df1 = pd.read_csv(args.fileA)
    print(f">>> Loading File B: {args.fileB}")
    df2 = pd.read_csv(args.fileB)
    ...
    dataset_folder = os.path.dirname(args.fileA)
    ...
    out_csv = os.path.join(out_dir, f"{args.name}_analysis_ready.csv")
